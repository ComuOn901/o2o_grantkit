...
class CitationExtractor:
    ...
    def _parse_citation_keys(self, citation_text: str) -> List[str]:
        ...
        for part in parts:
            ...
            elif part and not re.search(
                r"\s", part
            ):  # No spaces = likely a key
                keys.append(part)

        return keys

    def generate_citation_report(
        self, directory: Path, bibtex_keys: Set[str] = None
    ) -> CitationReport:
    ...
    def replace_citations_with_numbers(
        self, content: str, citation_order: List[str]
    ) -> str:
